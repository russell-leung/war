...
#MAXES OUT AT 5000 PLAYS PER GAME SO WE CAN READ DATA, SIMULATES 10 GAMES AND FINDS THE AVERAGE STATS BETWEEN THE GAMES
...
class Game:
  ...
  def check_cards(self):
    ...
    def check_cards_against_eachother(user_card_values):
      ...
      #Runs while both players have cards in their deck, if the game reaches 5000 plays, it will end the game
      while len(user_card_values[0]) != 0 or len(user_card_values[1]) != 0:
        ...
        #Change this to change the cap on how many plays the game will simulate before it maxes out
        if self.play_count == 5000:
          print("\nThis game has maxed out at 5000 turns, there is no winner.")
          return
...
total_user2_wins = 0
total_war_count = 0
total_user1_war_wins = 0
total_user2_war_wins = 0
...
for game in range(10):
  ...
  total_play_count += game_stats[0]
  total_user1_wins += game_stats[1]
  total_user2_wins += game_stats[2]
  total_war_count += game_stats[3]
  total_user1_war_wins += game_stats[4]
  total_user2_war_wins += game_stats[5]
...
#Averages the stats of 10 war games
print("AVERAGE OF TEN GAMES: \n")
print(f"Total Game Plays: {total_play_count/10}\nUser One Total Normal Play Wins: {total_user1_wins/10}\nUser Two Total Normal Play Wins: {total_user2_wins/10}\nTotal Number Of Wars: {total_war_count/10}\nUser One Total Wars Won: {total_user1_war_wins/10}\nUser Two Total Wars Won: {total_user2_war_wins/10}")